	...
	if(I%P==0) {
		...
		for(t=1;t<=T;t++) {
			...
			if(rank == 0) {
				...
				MPI_Send(&value[((M-1)*J+(J-1))*W+(W-1)], 1, MPI_FLOAT, (rank+1), 1, MPI_COMM_WORLD);
        			MPI_Recv(&v_next, 1, MPI_FLOAT, (rank+1), 1, MPI_COMM_WORLD, &status);
				
				if((M>1)&&(J>1)&&(W>1)){

					// 0
					value[(0*J+0)*W+0]=((2*value[(0*J+0)*W+0]+value[(0*J+1)*W+0]+value[(1*J+0)*W+0]+value[(0*J+0)*W+1])/5);
					// i == 0; j==0;
					for(w=1;w<W-1;w++){
						value[(0*J+0)*W+w]=((value[(0*J+0)*W+(w-1)]+2*value[(0*J+0)*W+w]+value[(0*J+0)*W+(w+1)]+value[(0*J+1)*W+w]+value[(1*J+0)*W+w])/6);
					}
					value[(0*J+0)*W+(W-1)]=((value[(0*J+0)*W+(W-2)]+2*value[(0*J+0)*W+(W-1)]+value[(0*J+1)*W+(W-1)]+value[(1*J+0)*W+(W-1)])/5);
					// i == 0; w == 0;
					for(j=1;j<J-1;j++){
