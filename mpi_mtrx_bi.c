...
/* Neste codigo foi utilizado uma matriz com alocação dinâmica, no qual I será o numero de linhas(entrado pelo usuário) e J o numero de colunas(entrado pelo usuario). 
	...
	if(I%P==0) {
		...
		for(t=1;t<=T;t++) {
			...
			if(rank == 0) {
				...
				MPI_Send(&value[M-1][J-1], 1, MPI_FLOAT, (rank+1), 1, MPI_COMM_WORLD);
				...
				if((M>1)&&(J>1)){
					...
					for(j=1;j<J-1;j++)
						value[0][j]=((value[0][j-1]+2*value[0][j]+value[0][j+1]+value[1][j])/5);
					value[0][J-1]=((value[0][J-2]+2*value[0][J-1]+value[1][J-1])/4);
					// entre 0 e M-1
					for(i=1;i<M-1;i++){
						value[i][0]=((2*value[i][0]+value[i][1]+value[i+1][0]+value[i-1][0])/5);
						for(j=1;j<J-1;j++)
							value[i][j]=((value[i][j-1]+2*value[i][j]+value[i][j+1]+value[i+1][j]+value[i-1][j])/6);
						value[i][J-1]=((value[i][J-2]+2*value[i][J-1]+value[i+1][J-1]+value[i-1][J-1])/5);
					}
					// M-1
					value[M-1][0]=((2*value[M-1][0]+value[M-1][1]+value[M-2][0])/4);
					for(j=1;j<J-1;j++)
						value[M-1][j]=((value[M-1][j-1]+2*value[M-1][j]+value[M-1][j+1]+value[M-2][j])/5);
					value[M-1][J-1]=((value[M-1][J-2]+2*value[M-1][J-1]+value[M-2][J-1]+v_next)/5);
				}
				...
				if((M>1)&&(J<=1)){
					...
					// entre 0 e M-1
					for(i=1;i<M-1;i++)
						value[i][0]=((value[i-1][0]+2*value[i][0]+value[i+1][0])/4);
					// M-1
					value[M-1][0]=((value[M-2][0]+2*value[M-1][0]+v_next)/4);
				}
				...
				if((M<=1)&&(J>1)){
					...
					// entre 0 e J-1
					for(j=1;j<J-1;j++)
						value[0][j]=((value[0][j-1]+2*value[0][j]+value[0][j+1])/4);
					// J-1
					value[0][J-1]=((value[0][J-2]+2*value[0][J-1]+v_next)/4);
				}
			...
			if((rank>0)&&(rank<(size-1))) {
				...
				if(M>1){
					// 0
					value[0][0]=((2*value[0][0]+value[0][1]+value[1][0]+v_previous)/5);
					for(j=1;j<J-1;j++)
						value[0][j]=((value[0][j-1]+2*value[0][j]+value[0][j+1]+value[1][j])/5);
					value[0][J-1]=((value[0][J-2]+2*value[0][J-1]+value[1][J-1])/4);
					// entre 0 e M-1
				...
				if((M<=1)&&(J<=1)){
					value[0][0]=((2*value[0][0]+value[0][1]+v_previous+v_next)/5);
				}
			}
			...
			if(rank == (size-1)) {
				...
				if(M>1){
					...
					for(j=1;j<J-1;j++)
						value[M-1][j]=((value[M-1][j-1]+2*value[M-1][j]+value[M-1][j+1]+value[M-2][j])/5);
					value[M-1][J-1]=((value[M-1][J-2]+2*value[M-1][J-1]+value[M-2][J-1])/4);
				}
